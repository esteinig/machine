...
The fundamental building blocks of neural networks are neurons: logistic units.
The goal of logistic regression is to predict the result of a random experiment with a binary outcome (e.g. TRUE/FALSE, PASS/FAIL, 0/1), i.e. a Bernoulli trial that has probability p of success
Because we are trying to predict a probability p, which lies in the range [0,1], linear regression is not an appropriate model: for x?(-8,8) linear regression models for p would predict values in the interval (-8,8)
Therefore, rather than modelling the probability p directly with a linear regression model, we instead model the log odds
In logistic regression we are modelling the log odds with a straight line (or plane/hyperplane)
If we let y ^=P(y=1|x), i.e. the probability that a random variable y is equal to 1 given the data x:
log??y ^/(1-y ^ )=w^T x+b?=z
Rearranging this expression for the probability of success y ^ we get
y ^=s(z)=1/(1+exp??(-z)? )
